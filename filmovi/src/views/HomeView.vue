  ...
  <div class="home">
    <v-carousel cycle show-arrows-on-hover>
      <template v-slot:prev="{ on, attrs }">
        <v-btn color="#FF7704" v-bind="attrs" v-on="on">Prethodno</v-btn>
      </template>
      <template v-slot:next="{ on, attrs }">
        <v-btn color="#3f51b5" v-bind="attrs" v-on="on">Sljedece</v-btn>
      </template>
      <v-carousel-item
...
export default {
  ...
  methods: {
    dohvatiNajpopularnije: function () {
      const filmovi = ["Thor", "Minions", "Toronto", "Batman"];
      for (const film of filmovi) {
        this.axios
          .get("https://www.omdbapi.com/?apikey=421c5dbb&y=2022&t=" + film)
          .then((response) => {
            this.filmovi.push(response.data);
          });
      }
    },
  },
